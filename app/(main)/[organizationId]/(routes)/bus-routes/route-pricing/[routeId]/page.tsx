...
const RoutePricing = () => {
  ...
  const [forwardCombo, setForwardCombo] = useState({});
  const [backwardCombo, setBackwardCombo] = useState({});
  ...
  const POST_RegisterSegmentTickets = async () => {
    const segmentCombo = { ...forwardCombo, ...backwardCombo };

    try {
      const response = await api.post(
        `/admin/register-segment-tickets/${organizationId}`,
        { segments: segmentCombo }
      );
